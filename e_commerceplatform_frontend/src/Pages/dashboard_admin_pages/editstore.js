...
const EditStore = () => {
        ...
        const [sellers, setSellers] = useState([]); ///جلب  المتاجر
            ...
            const fetchData = async() => {
                try {
                    ...
                    if (sellerResponse.data.status === 200) {
                        setSellers(sellerResponse.data.seller);
                    }

                    if (storeResponse.data.status === 200) {
                        const storeData = storeResponse.data.store; // تغيير اسم المتغير
                        setData(storeData);
                        setName(storeData.name || '');
                        setAddress(storeData.address || '');
                        setDescription(storeData.description || '');
                        setPersonalNumber(storeData.personalNumber || '');
                        setPhone(storeData.phone || '');
                        setType(storeData.type || '');
                        setTime(storeData.openTime || '');
                        setSelectedSeller(storeData.seller_id || '');
                        console.log(storeResponse.data.store);
                    }
                } catch (error) {
                    console.error(error);
                }
